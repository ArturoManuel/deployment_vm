...
def list_subnets(neutron_endpoint, token, network_id=None):
...
def list_ports(neutron_endpoint, token, network_id=None):
...
def update_project_quota(nova_endpoint, token, project_id, instance_quota):
    url = f"{nova_endpoint}/os-quota-sets/{project_id}"
    headers = {'Content-type': 'application/json', 'X-Auth-Token': token}
    ...
    if r.status_code == 200:
        print(f"Cuota de instancias establecida a {instance_quota} para el proyecto con ID '{project_id}'.")
    else:
        print(f"Error al actualizar la cuota de instancias: {r.status_code} - {r.text}")
        raise Exception(f"Error al actualizar la cuota del proyecto '{project_id}'")
    
    return r
def list_networks(neutron_endpoint, token, project_id=None):
...
def delete_project(auth_endpoint, token, project_id):
    url = f"{auth_endpoint}/projects/{project_id}"
    headers = {'Content-type': 'application/json', 'X-Auth-Token': token}

    response = requests.delete(url=url, headers=headers)
    return response

def list_roles(auth_endpoint, token):
    url = f"{auth_endpoint}/roles"
    headers = {'Content-type': 'application/json', 'X-Auth-Token': token}
